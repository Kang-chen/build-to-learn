...
> v2.13（2026-08-12 S3 · 经验连续的通关题）：通关题默认取自用户刚参与的真实修改、决策、失败或验证；“输入包完整”不能替代经验连续性。只有学习目标本来就是迁移、且陌生领域前置已经建立时，才引入跨领域新情境；出题越界则作废、不评分并换回当前项目。
> v2.12（2026-08-12 S3 · 双轴兼容）：明确学习目标与节奏档位是两条轴；Quick / Standard / Deep 只改变背景、解释和证据密度，不反转“用户掌判断、AI 扛执行”。同步清理 reference 与中英文教程中的旧动作级规则。
...
**节奏档位和学习目标是两条轴。** “只要跑通 / 能讲出原理 / 能迁移”决定最后要掌握到哪；Quick / Standard / Deep 决定本轮需要多少背景、解释、对照证据和交互。想学到迁移不等于每一步都要 Deep；高风险任务即使只求跑通，也不能省掉必要边界。

| 节奏 | 适用信号 | 会增加什么 | 不会改变什么 |
|---|---|---|---|
| **Quick（默认）** | 既有系统、低风险、可撤销、历史背景足够 | 最小路径图、关键边界、自动验证 | 用户仍掌有后果的判断；AI 连续做机械动作 |
| **Standard** | 开工前缺少一个安全推进所必需的基础，且最小背景不能一句补足；或局部异常已出现、需要多轮证据才能收敛 | 对相关边界增加解释、对照和一次必要观察 | 不因“可能存在隐藏边界”预先扩张；不扩写无关背景、不把命令交给用户 |
| **Deep** | 安全/隐私、不可逆、高成本，或多个陌生机制互相作用 | 完整相关部件图、边界实验、风险与失败证据 | 机械执行仍由 AI 负责；用户只掌决策、不可替代体验和验收 |

三档不是三套从头重跑的流程。先选能安全推进的最轻档；证据触发时只升级相关局部，满足收敛条件后回到原档。用户明确要求加深或加快时立即调整，但风险证据不能因“加快”被删掉。

“只要跑通”不是“不想学”：它仍在 Build to Learn 内，至少要学会判断是否真的跑通，并保留风险所需证据。只有用户明确说不想学习、只要成品，才退出本流程。
...
3. **验收 + 撞墙讲透**：AI 写完，一句话讲清这段代码替哪个决策干活，然后**直接跑**（2026-07-30 v2.3：废除「写完先预判再跑」的闸——让用户对没跑过的代码空想坑在哪，实测产出茫然不产出认知）。跑出反常或撞墙后，AI 展示现象并当场讲透因果；只有现象必须由用户亲自体验时，才由用户操作并反馈。用户主动给过解释的，认真对着证据校。AI 报"做好了"时用户能判断真假，靠真实验收和通关大考，不靠施工期问答。真源在此，`2-施工.md` 交棒节奏指回这里。
...
用户只想要东西、**明确不想学**（「别教我，直接做完」）→ 这个 skill 不适合，按普通方式帮他做。用户选择学习目标“只要跑通”仍适用本 skill，不得据此退出或跳过风险证据。
